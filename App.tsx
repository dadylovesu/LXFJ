...
const App: React.FC = () => {
  ...
  const handleCreateCollage = async (files: File[], rows: number, cols: number, targetAr: string) => {
      setIsCollageEditorOpen(false); 
      setGenerationStep("正在优化拼贴素材...");
      setIsGenerating(true);
      
      try {
          const stitchedBase64 = await stitchImages(files, rows, cols, targetAr);
  ...
  const handleDownloadBatch = async () => {
      ...
      const zip = new JSZip();
      const folder = zip.folder("OrangeStudio_renders");
      ...
      try {
          ...
          a.href = url;
          a.download = `OrangeStudio_Workflow_${Date.now()}.zip`;
  ...
  return (
    <div className="flex h-screen w-screen overflow-hidden bg-cine-black text-zinc-400 font-sans">
      ...
      <CollageEditor 
        ...
        onSave={(files, r, c, ar) => handleCreateCollage(files, r, c, ar)}
            ...
            <div className="absolute inset-0 bg-cine-black/90 backdrop-blur-xl z-50 flex flex-col items-center justify-center space-y-8 pointer-events-none animate-in fade-in duration-500">
                 <div className="relative scale-110">
                    ...
                    <div className="absolute inset-0 flex items-center justify-center">
                        <div className="w-3 h-3 bg-cine-accent rounded-full shadow-[0_0_20px_#FF7A00]"></div>
                    </div>
                 </div>
