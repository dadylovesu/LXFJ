...
const App: React.FC = () => {
  ...
  const createEmptyProject = (name: string): ProjectState => ({
    id: crypto.randomUUID(), name, images: [], assets: [], gridSize: 2,
    panelAspectRatio: PanelAspectRatio.P16_9, imageSize: ImageSize.K4,
    prompt: '', stylePrompt: '', styleRefImage: null, panelPrompts: [],
    activeCollage: null, scriptGroups: []
  });
  ...
  const captureCurrentStateAsProject = useCallback((): ProjectState | null => {
    ...
    return {
        ...current, assets, images, gridSize, panelAspectRatio, imageSize,
        prompt, stylePrompt, styleRefImage, panelPrompts, activeCollage, scriptGroups
    };
  ...
  useEffect(() => {
    ...
    saveTimeoutRef.current = setTimeout(async () => {
        ...
        } catch (err) { console.error("Autosave failed:", err); } finally { isSavingRef.current = false; }
    }, 3000);
  ...
  const rebuildGridImage = async (slices: string[], rows: number, cols: number): Promise<string> => {
    const loadImage = (url: string): Promise<HTMLImageElement> => new Promise((res, rej) => {
        const img = new Image();
        img.onload = () => res(img);
        img.onerror = () => rej(new Error(`Failed to load slice: ${url.slice(0, 20)}...`));
        img.src = url;
    });

    try {
        const loadedImages = await Promise.all(slices.map(loadImage));
  ...
  const handleSwapSlices = async (sourceId: string, sourceIdx: number, targetId: string, targetIdx: number) => {
    ...
    try {
      ...
      setSuccessMsg("分镜互换成功");
    } catch (e) {
      setError("互换失败，图像合成引擎异常");
    } finally {
  ...
  const handleSwitchProject = (id: string) => {
      ...
      if (p) { setActiveProjectId(id); loadProjectToState(p); }
  };
  const handleRenameProject = (id: string, name: string) => { setProjects(prev => prev.map(p => p.id === id ? { ...p, name } : p)); };
  const handleDeleteProject = (id: string) => {
      if (projects.length <= 1) return;
      const newList = projects.filter(p => p.id !== id);
      setProjects(newList);
      if (activeProjectId === id) { setActiveProjectId(newList[0].id); loadProjectToState(newList[0]); }
  };
  ...
  const handleExportFull = async () => {
      ...
      } catch (e: any) { setError("导出失败: " + e.message); } finally { setIsGenerating(false); }
  };
  const handleSaveProjectScript = async () => {
      ...
      } catch (e) { setError("导出脚本失败：数据量超过限制。"); }
  };
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => { if ((e.ctrlKey || e.metaKey) && e.key === 's') { e.preventDefault(); handleSaveProjectScript(); } };
  ...
  const undo = useCallback(() => {
    if (history.length > 0) {
      ...
      setImages(prev); setHistory(prevStack => prevStack.slice(0, -1)); setSelectedImageId(undefined);
    }
  }, [history]);
  ...
  const handleAddAsset = (files: FileList, category: AssetCategory) => {
    Array.from(files).forEach(async (file) => {
      ...
      const newAsset: Asset = { id: crypto.randomUUID(), previewUrl: base64, type: 'image', category, index: (category === 'role' || category === 'prop') ? categoryCount + 1 : undefined };
  ...
  const handleGenerate = async () => {
    setError(null); setIsGenerating(true); setGenerationStep("正在启动渲染引擎...");
    ...
    try {
      ...
      if (parentNode) {
          startX = (parentNode.position?.x || 0); startY = (parentNode.position?.y || 0) + 480; previousContextImage = parentNode.fullGridUrl || parentNode.url;
      } else {
      ...
      const referenceData = assets.map(asset => ({ data: asset.previewUrl.split(',')[1], mimeType: 'image/png', category: asset.category as any, roleIndex: asset.index }));
      setGenerationStep("分镜渲染与动线分析同步执行中...");
      const [finalResult, cameraMove] = await Promise.all([
        generateMultiViewGrid(prompt, gridSize, panelAspectRatio, aspectRatio, imageSize, referenceData, previousContextImage, activeCollage ? undefined : panelPrompts, activeCollage || undefined, stylePrompt, styleRefImage || undefined),
        generateCameraMovement(prompt)
      ]);
      const finalNode: GeneratedImage = { id: crypto.randomUUID(), url: finalResult.fullImage, fullGridUrl: finalResult.fullImage, prompt, stylePrompt, textData: prompt, assetIds: assets.map(a => a.id), aspectRatio, panelAspectRatio, timestamp: Date.now(), nodeType: 'render', parentId: parentNode?.id, position: { x: startX, y: startY }, cameraDescription: cameraMove, slices: finalResult.slices, panelPrompts: [...panelPrompts], sliceHistory: {}, gridRows: gridSize, gridCols: gridSize };
      updateImagesWithHistory([...images, finalNode]);
      setSelectedImageId(finalNode.id);
    } catch (err: any) { setError(err.message || "生成失败"); } finally { setIsGenerating(false); setGenerationStep(""); }
  };
  const handleLensLabRender = async (anchorImage: string, queue: LensLabParams[]) => {
      setError(null); setIsGenerating(true); setGenerationStep("Lens Lab: 正在执行3D多角度一致性模拟...");
      try {
        const rootNodes = images.filter(i => !i.parentId);
        const startX = rootNodes.length === 0 ? 100 : (rootNodes[rootNodes.length-1].position?.x || 100) + 420;
        const result = await generateLensLabSequence(anchorImage.split(',')[1], gridSize, queue, panelAspectRatio, aspectRatio, imageSize, stylePrompt, styleRefImage || undefined);
        const node: GeneratedImage = { id: crypto.randomUUID(), url: result.fullImage, fullGridUrl: result.fullImage, prompt: "Lens Lab Sequence Render", textData: "Lens Lab Consistency Render\n\nGenerated Panels:\n" + result.panelPrompts.join('\n'), aspectRatio, panelAspectRatio, timestamp: Date.now(), nodeType: 'lens_lab', position: { x: startX, y: 100 }, cameraDescription: "Multi-angle spherical orbit sequence.", slices: result.slices, panelPrompts: result.panelPrompts, gridRows: gridSize, gridCols: gridSize };
        updateImagesWithHistory([...images, node]); setSelectedImageId(node.id);
      } catch (err: any) { setError(err.message || "Lens Lab 渲染失败"); } finally { setIsGenerating(false); setGenerationStep(""); }
  };
  const handleEditSlice = async (imageId: string, sliceIndex: number, editPrompt: string, usePro: boolean, refImage?: string, targetImageSize: ImageSize = ImageSize.K1) => {
    setIsGenerating(true); setGenerationStep("正在执行单格无缝重绘...");
    try {
      const image = images.find(img => img.id === imageId);
      if (!image || !image.slices) return;
      const model = usePro ? 'gemini-3-pro-image-preview' : 'gemini-2.5-flash-image';
      const newSliceUrl = await editImage(image.slices[sliceIndex], editPrompt, model, image.panelAspectRatio || image.aspectRatio, refImage, targetImageSize, stylePrompt, styleRefImage || undefined);
  ...
  const handleApplyScripts = (summary: string, scripts: string[]) => {
      setPrompt(summary); setPanelPrompts(scripts);
      const count = scripts.length;
      if (count > gridSize * gridSize) { const nextSide = Math.ceil(Math.sqrt(count)); setGridSize(nextSide); }
  };
  const handleDownloadZip = async () => {
    const selected = images.find(i => i.id === selectedImageId);
    if (!selected) return;
    setIsGenerating(true); setGenerationStep("正在准备镜头素材打包...");
    try {
      const zip = new JSZip();
      ...
      if (folder) {
          if (selected.slices) { selected.slices.forEach((s, i) => { folder.file(`渲染图_Shot${shotIndex}_Panel${i+1}.png`, base64ToBlob(s)); }); }
          folder.file(`全景宫格_Shot${shotIndex}.png`, base64ToBlob(selected.fullGridUrl || selected.url));
      }
      const content = await zip.generateAsync({ type: "blob" });
      const link = document.createElement('a'); link.href = URL.createObjectURL(content); link.download = `${folderName}_素材包.zip`; link.click();
      setSuccessMsg("镜头素材包已导出");
    } catch (err) { alert("ZIP 打包失败"); } finally { setIsGenerating(false); setGenerationStep(""); }
  };
  ...
  return (
    <div className="flex h-screen w-screen overflow-hidden bg-cine-black text-zinc-400 font-sans">
      <aside className="w-[340px] flex flex-col border-r border-cine-border bg-cine-dark z-20 shadow-2xl">
        <ProjectManager projects={projects} activeProjectId={activeProjectId} onSwitchProject={handleSwitchProject} onNewProject={handleNewProject} onImportProject={handleImportProject} onExportFull={handleExportFull} onSaveIncremental={handleSaveProjectScript} onRenameProject={handleRenameProject} onDeleteProject={handleDeleteProject} />
        <div className="p-4 border-b border-cine-border bg-cine-black/50 flex justify-between items-center">
            <h1 className="text-white text-[10px] font-bold tracking-[0.15em] uppercase font-mono flex items-center gap-2.5"><span className="w-2 h-2 bg-cine-accent rounded-[1px]"></span>创作中心 (WORKSPACE)</h1>
        ...
        <div className="flex-1 flex flex-col p-4 gap-7 overflow-y-auto custom-scrollbar">
            <AssetBay assets={assets} onAddAsset={handleAddAsset} onRemoveAsset={handleRemoveAsset} onSelectAsset={(a) => { setSelectedAssetId(a.id); setSelectedImageId(undefined); }} selectedAssetId={selectedAssetId} onOpenCollage={() => setIsCollageEditorOpen(true)} />
            <div className="px-2">
                {activeCollage ? (
                  <div className="p-3 bg-cine-accent/10 border border-cine-accent/30 rounded-sm space-y-2">
                     <div className="flex items-center justify-between"><span className="text-[10px] text-cine-accent font-bold uppercase tracking-widest flex items-center gap-2"><LayoutGrid size={12} /> 镜头组参考 (ACTIVE)</span><button onClick={() => setActiveCollage(null)} className="text-cine-accent hover:text-white"><XIcon size={12} /></button></div>
            ...
            <div className="px-1 space-y-6">
                ...
                <DirectorDeck gridSize={gridSize} setGridSize={setGridSize} aspectRatio={aspectRatio} panelAspectRatio={panelAspectRatio} setPanelAspectRatio={setPanelAspectRatio} imageSize={imageSize} setImageSize={setImageSize} prompt={prompt} setPrompt={setPrompt} stylePrompt={stylePrompt} setStylePrompt={setStylePrompt} styleRefImage={styleRefImage} setStyleRefImage={setStyleRefImage} onGenerate={handleGenerate} onStop={() => setIsGenerating(false)} isGenerating={isGenerating} onEnhancePrompt={async () => setPrompt(await enhancePrompt(prompt))} onGenerateCamera={() => setIsCameraEditorOpen(true)} onOpenScriptDeconstruct={() => setIsScriptEditorOpen(true)} isContinuing={!!selectedImageId} selectedImage={selectedImage} onDeselect={() => { setSelectedImageId(undefined); setSelectedAssetId(undefined); }} isCollageActive={!!activeCollage} />
            </div>
        </div>
      </aside>
      <main className="flex-1 relative bg-cine-black">
        <Canvas images={images} onSelect={(i) => { setSelectedImageId(i.id); setSelectedAssetId(undefined); }} selectedId={selectedImageId} onDelete={handleDeleteNode} onUpdateNodePosition={handleUpdateNodePosition} onDownloadAll={handleDownloadZip} assets={assets} onDeselectAll={() => { setSelectedImageId(undefined); setSelectedAssetId(undefined); }} onSwapSlices={handleSwapSlices} />
        {isGenerating && (
        ...
        <CameraEditor isOpen={isCameraEditorOpen} onClose={() => setIsCameraEditorOpen(false)} rows={gridSize} cols={gridSize} mainPrompt={prompt} initialPrompts={panelPrompts} onSave={handleSaveCameraLogic} selectedImage={selectedImage || undefined} onRegenSlice={(idx, p) => handleEditSlice(selectedImageId!, idx, p, true)} isGenerating={isGenerating} />
        ...
        <ScriptEditor isOpen={isScriptEditorOpen} onClose={() => setIsScriptEditorOpen(false)} defaultPanelCount={gridSize * gridSize} scriptGroups={scriptGroups} onUpdateScriptGroups={setScriptGroups} onApplyScripts={handleApplyScripts} />
      ...
      <aside className="w-[400px] bg-cine-dark border-l border-cine-border z-20">
         <Inspector selectedImage={selectedImage} selectedAsset={assets.find(a => a.id === selectedAssetId) || null} onClose={() => { setSelectedImageId(undefined); setSelectedAssetId(undefined); }} onEditSlice={handleEditSlice} onRevertSlice={(imgId, sIdx, hIdx) => {
                const newImages = images.map(img => {
                  if (img.id === imgId) {
