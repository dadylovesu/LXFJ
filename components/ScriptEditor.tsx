...
interface ScriptEditorProps {
  ...
  scriptGroups: ScriptGroup[]; // 接收 Props
  onUpdateScriptGroups: (groups: ScriptGroup[]) => void; // 回调更新
}
...
export const ScriptEditor: React.FC<ScriptEditorProps> = ({ 
  ...
  const saveAutoGroup = (lines: string[]) => {
      ...
      onUpdateScriptGroups([newGroup, ...scriptGroups]);
  };
                ...
                {activeTab === 'input' ? (
                    <>
                        ...
                        {activeMode === 'text' ? (
                            <div className="space-y-6 animate-in fade-in duration-300">
                                ...
                                <div className="space-y-4">
                                    <label className="text-[10px] uppercase text-zinc-500 font-bold tracking-[0.2em]">常用指令库</label>
                                    <div className="space-y-2">
                                        {savedPrompts.map(p => (
                                            <div key={p.id} className="bg-black/40 border border-zinc-800 p-2 text-[9px] font-mono text-zinc-400 truncate cursor-pointer hover:border-zinc-600 flex justify-between group" onClick={() => setInstruction(p.content)}>
                                                <span className="truncate">{p.title}</span>
                                                <button onClick={(e) => { e.stopPropagation(); handleRemoveSavedPrompt(p.id); }} className="opacity-0 group-hover:opacity-100 text-zinc-600 hover:text-red-500 transition-all"><Trash2 size={10} /></button>
                                            </div>
                                        ))}
                                    </div>
                                </div>
                                    ...
                                    {!videoPreviewUrl ? (
                                        ...
                                        <div className="space-y-3">
                                            <div className="aspect-video bg-black rounded-sm border border-zinc-800 overflow-hidden relative group">
                                                ...
                                                <button onClick={() => { setVideoBase64(null); setVideoPreviewUrl(null); }} className="absolute top-2 right-2 p-1.5 bg-black/60 text-white rounded-full opacity-0 group-hover:opacity-100 transition-opacity hover:bg-red-500">
                                                    <Trash2 size={12} />
                                                </button>
                                            </div>
                                        </div>
                    ...
                    <div className="space-y-4">
                         {scriptGroups.length === 0 && (
                             <div className="py-20 text-center text-zinc-700 font-mono text-[10px] uppercase tracking-widest">
                                 该项目暂无历史记录
                             </div>
                         )}
