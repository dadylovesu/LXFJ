...
export const ScriptEditor: React.FC<ScriptEditorProps> = ({ 
  ...
  const handleDownloadGroup = (group: ScriptGroup) => {
      let content = `工程脚本导出: ${group.name}\n`;
      content += `导出时间: ${new Date(group.timestamp).toLocaleString()}\n`;
      content += `--------------------------------------------------\n`;
      content += `【剧情梗概/指令】\n${group.summary || '无梗概'}\n\n`;
      content += `【分镜详细描述】\n`;
      group.scripts.forEach((script, index) => {
          content += `${index + 1}. ${script}\n\n`;
      });
      content += `--------------------------------------------------\n`;
      content += `Generated by OrangeStudio Cinematic Agent`;

      const blob = new Blob([content], { type: 'text/plain;charset=utf-8' });
      const url = URL.createObjectURL(blob);
      const link = document.createElement('a');
      link.href = url;
      link.download = `${group.name.replace(/\s+/g, '_')}_分镜脚本.txt`;
      link.click();
      URL.revokeObjectURL(url);
  };

  const handleDownloadAllHistory = () => {
      if (scriptGroups.length === 0) return;
      let content = `OrangeStudio 完整脚本历史记录导出\n`;
      content += `导出时间: ${new Date().toLocaleString()}\n`;
      content += `==================================================\n\n`;

      scriptGroups.forEach((group, gIdx) => {
          content += `### 记录 ${gIdx + 1}: ${group.name}\n`;
          content += `时间: ${new Date(group.timestamp).toLocaleString()}\n`;
          content += `梗概: ${group.summary || '无'}\n`;
          content += `分镜列表:\n`;
          group.scripts.forEach((script, sIdx) => {
              content += `  [${sIdx + 1}] ${script}\n`;
          });
          content += `\n--------------------------------------------------\n\n`;
      });
      ...
      const link = document.createElement('a');
      link.href = url;
      link.download = `完整脚本历史_${new Date().getTime()}.txt`;
      link.click();
      URL.revokeObjectURL(url);
  };
                ...
                {activeTab === 'input' ? (
                                        ...
                                        <div className="space-y-3">
                                            ...
                                            <div className="p-3 bg-cine-accent/5 border border-cine-accent/20 rounded-sm flex items-start gap-3">
                                                ...
                                                <p className="text-[9px] text-zinc-400 font-mono leading-relaxed">
                                                    视频反推将自动识别镜头切换点（Shot Cuts），并按导演视角反推导出专业的景别、构图及动态描述。
                                                </p>
                                            </div>
                                        </div>
                                    )}
                    ...
                    <div className="space-y-4">
                         <div className="flex items-center justify-between px-1 mb-2">
                             <span className="text-[9px] font-mono text-zinc-500 uppercase tracking-widest">历史记录 ({scriptGroups.length})</span>
                             {scriptGroups.length > 0 && (
                                 <button 
                                    onClick={handleDownloadAllHistory}
                                    className="text-[9px] text-cine-accent hover:underline font-mono flex items-center gap-1"
                                 >
                                    <Download size={10} /> 导出全部
                                 </button>
                             )}
                         </div>
                         {scriptGroups.map(group => (
                             <div key={group.id} className="bg-black/40 border border-zinc-800 rounded-sm p-3 hover:border-cine-accent/30 transition-all cursor-pointer group/item" onClick={() => handleLoadGroup(group)}>
                                     ...
                                     {editingGroupId === group.id ? (
                                         <div className="flex-1 flex gap-2" onClick={e => e.stopPropagation()}>
                                             <input 
                                                ...
                                                onKeyDown={e => {
                                                    if (e.key === 'Enter') handleRenameGroup(group.id, editGroupNameValue);
                                                    if (e.key === 'Escape') setEditingGroupId(null);
                                                }}
                                                className="flex-1 bg-black border border-cine-accent text-white text-[10px] font-mono px-2 py-0.5 rounded-sm outline-none"
                                             />
                                             <button onClick={() => handleRenameGroup(group.id, editGroupNameValue)} className="text-cine-accent"><Check size={14}/></button>
                                         </div>
                                     ) : (
                                         <span className="text-[10px] text-zinc-200 font-bold font-mono truncate">{group.name}</span>
                                     )}
                                     
                                     <div className="flex items-center gap-2 opacity-0 group-hover/item:opacity-100 transition-opacity ml-2">
                                         {editingGroupId !== group.id && (
                                             <button 
                                                onClick={(e) => { 
                                                    e.stopPropagation(); 
                         ...
                         {scriptGroups.length === 0 && (
                             <div className="py-20 text-center">
                                 <p className="text-[9px] font-mono text-zinc-700 uppercase tracking-[0.2em]">暂无历史记录</p>
                             </div>
                         )}
                    </div>
                )}
